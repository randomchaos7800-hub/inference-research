...
Unit of analysis: one prediction (25 cases x 3 readers = 75 items). Raters: Dino (hand), Claude, gpt-5.6-luna, Grok (grok-4-fast via the grok CLI). Labels: binary correct/incorrect.
Cohen's kappa (unweighted, two categories) for each rater pair; Fleiss' kappa for all four raters together.
...
grok = {r["item"]: int(r["correct"]) for r in csv.DictReader(open("rater_grok.csv"))}

rows = []
for item, (t, rd) in sorted(key.items()):
    d, c, l, g = int(hj[t][rd + "_hand"]), claude[item], luna[item], grok[item]
    rows.append(dict(item=item, test_id=t, reader=rd, score_exact=hj[t][rd + "_exact"], dino_hand=d,
                     claude_blind=c, gpt56luna_blind=l, grok_blind=g, majority=int(d + c + l + g >= 3), unanimous=int(d == c == l == g), note=notes[item]))
...
D = [r["dino_hand"] for r in rows]; C = [r["claude_blind"] for r in rows]; L = [r["gpt56luna_blind"] for r in rows]; G = [r["grok_blind"] for r in rows]
for name, (a, b) in {"dino_vs_claude": (D, C), "dino_vs_luna": (D, L), "dino_vs_grok": (D, G), "claude_vs_luna": (C, L), "claude_vs_grok": (C, G), "luna_vs_grok": (L, G)}.items():
    po, k = cohen(a, b); print(f"{name}: agreement {po:.3f}  cohen_kappa {k:.3f}")
print(f"fleiss_kappa (4 raters): {fleiss([D, C, L, G]):.3f}")
